...
#
# MongoBackup2S3
# Automatically backup mongodb and upload it to S3
# More Info: https://github.com/silverbux/mongobackup2s3
#
# Note, most of the codes are part of MongoDB Backup Script VER. 0.20,
# Only additional S3 upload feature. All All necessary credits are given.
...
if [ "$DBUSERNAME" ]; then
    OPT="$OPT -u $DBUSERNAME -p $DBPASSWORD"
...
dbdump () {
    # mongodump -h $DBHOST:$DBPORT -d $DBNAME -u "${DBUSERNAME}" -p "${DBPASSWORD}" --out=$1
...
COPYTOFILE="${DATE}.${DOW}.tgz"

COPYFROMFILE="${FILE}.tgz"

cp $COPYFROMFILE $COPYTOFILE

FILE_TO_UPLOAD=$COPYTOFILE

find $FILE_TO_UPLOAD -type f -print0 | while IFS= read -r -d '' i; do
  FILE="$(perl -MURI::Escape -e 'print uri_escape($ARGV[0],"^A-Za-z0-9\-\._~\/");' "$i")"
  RESOURCE="/${BUCKET}/${FILE}"
  DATE_VALUE="$(LC_ALL=C date -u +"%a, %d %b %Y %X %z")"
  STRING_TO_SIGN="HEAD\n\n\n${DATE_VALUE}\n${RESOURCE}"
